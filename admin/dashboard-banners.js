...
/* ══════════════════════════════════════════════
   RENDER — mỗi banner là 1 card độc lập, toggle
   dùng data-index để tránh lỗi closure stale
   ══════════════════════════════════════════════ */
        ...
        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;user-select:none;">
          <div style="position:relative;width:44px;height:24px;flex-shrink:0;">
            <input
              type="checkbox"
              data-banner-index="${i}"
              class="banner-visible-cb"
              ${b.visible ? 'checked' : ''}
              style="opacity:0;width:0;height:0;position:absolute;"
            >
            <!-- Track: click vào track → trigger checkbox -->
          ...
          <span class="banner-visible-label" data-banner-index="${i}" style="
            ...
            color:${b.visible ? 'var(--primary)' : 'var(--text-muted)'};
      ...
      <div style="padding:20px 22px;display:grid;grid-template-columns:1fr auto;gap:16px;align-items:flex-start;${!b.visible ? 'opacity:.5;' : ''}transition:opacity .2s;" class="banner-body" data-banner-index="${i}">
        ...
        <div>
          ...
          <div class="banner-preview-wrap" data-banner-index="${i}" style="
            width:180px;height:90px;border-radius:10px;
            border:1.5px solid var(--border);overflow:hidden;
            background:var(--bg);display:flex;align-items:center;justify-content:center;
          ">
            ${b.url
              ? `<img src="${escBannerAttr(b.url)}" alt="Preview"
...
/* ══════════════════════════════════════════════
   EVENTS — gắn sau render, không dùng inline
   onclick để tránh lỗi index bị cache
   ══════════════════════════════════════════════ */
function attachBannerEvents() {
  /* Toggle: click vào track → toggle checkbox tương ứng */
  ...
  /* Checkbox change → cập nhật state + visual */
  ...
  /* URL input → live preview + cập nhật state */
