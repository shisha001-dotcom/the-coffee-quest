/* ══════════════════════════════════════════════
   ...
   File: admin/dashboard-accounts.js
   Chức năng: Superadmin tạo / sửa / xóa tài khoản
   quản trị (bảng admin_users trên Supabase).
   Mật khẩu luôn được hash SHA-256 phía client
   trước khi gửi lên Supabase — không bao giờ
   lưu plaintext.
   ══════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════
   SHA-256 HASH (giống hệt admin/login.html)
   ══════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════
   INJECT MENU ITEM — chỉ hiện với superadmin
   ══════════════════════════════════════════════ */
(function injectAccountsMenu() {
  if (!isSuperAdmin) return; // Editor không thấy mục này
  ...
  const tryInject = () => {
    ...
    item.id = 'accountsMenuItem';
    item.style.cssText = 'cursor:pointer;';
    item.innerHTML = `<span>👤</span> Quản lý tài khoản`;
    item.addEventListener('click', () => showAccountsPage());
    if (settingsItem) target.insertBefore(item, settingsItem);
...
/* ══════════════════════════════════════════════
   INJECT PAGE HTML — chỉ inject với superadmin
   ══════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════
   SHOW PAGE (có guard: chặn nếu không phải superadmin,
   phòng trường hợp bị gọi thủ công qua console)
   ══════════════════════════════════════════════ */
window.showAccountsPage = function () {
  if (!isSuperAdmin) {
    alert('Chỉ Super Admin mới có quyền truy cập Quản lý tài khoản.');
    return;
  }
  if (typeof window.__showPage === 'function') {
    window.__showPage('accountsPage');
  } else {
    ['dashboardPage', 'boardgamesPage', 'drinksPage', 'analyticsPage', 'chatAdminPage', 'bannersPage', 'accountsPage']
      .forEach(id => { const el = document.getElementById(id); if (el) el.style.display = 'none'; });
    document.getElementById('accountsPage').style.display = '';
  }
  document.querySelectorAll('.menu-item, .menu-item-parent').forEach(el => el.classList.remove('active'));
  document.getElementById('accountsMenuItem')?.classList.add('active');
  loadAccounts();
};
...
function renderAccountsTable(list) {
  ...
  tbody.innerHTML = list.map(acc => {
    ...
    return `<tr>
      <td>
        <div class="game-info">
          <div style="width:40px;height:40px;border-radius:50%;background:${roleBg};display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;color:#fff;flex-shrink:0;">${initial}</div>
          <div>
            <div class="game-name">${escAcc(acc.display_name || acc.username)}${isSelf ? ' <span style="font-size:11px;color:var(--text-muted);font-weight:500;">(bạn)</span>' : ''}</div>
            <div class="game-id">@${escAcc(acc.username)}</div>
          </div>
        </div>
      </td>
...
function openEditAccount(id) {
  ...
  document.getElementById('accountUsername').disabled = true; // tránh đổi username gây lệch session cũ
  ...
  delBtn.style.display = acc.id === currentSession.id ? 'none' : 'inline-flex'; // không tự xóa chính mình
  document.getElementById('accountModal').classList.remove('hidden');
}

/* ══════════════════════════════════════════════
   SAVE (INSERT / UPDATE) — luôn hash mật khẩu
   ══════════════════════════════════════════════ */
async function saveAccount() {
  ...
  try {
    if (!id) {
      /* ── Kiểm tra trùng username trước khi tạo mới ── */
      ...
      if (password) payload.password_hash = await sha256(password); // chỉ hash & cập nhật khi có nhập mật khẩu mới
...
function bindAccountEvents() {
  ...
  /* Event delegation cho nút Sửa trong bảng */
