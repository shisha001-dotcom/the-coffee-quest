/* ══════════════════════════════════════════════
   ...
   ⚠️ SỬA (khắc phục lỗi trang Settings không tải được — 2026-08-12):
   File này TRƯỚC ĐÂY bị lưu SAI đường dẫn tại
   admin/modules/dashboard-settings.js (thiếu thư mục con "settings/"),
   trong khi admin/core/dashboard-auth.js::SCRIPT_SEQUENCE lại tham
   chiếu "./modules/settings/dashboard-settings.js" — sai lệch này
   khiến trình duyệt 404 khi cố tải file, nên IIFE claimSettingsPlaceholder()
   và registerPage() bên dưới KHÔNG BAO GIỜ CHẠY → mục "⚙️ Settings"
   trên sidebar không có onclick nào được gán → bấm vào không có phản
   ứng gì (im lặng, không báo lỗi rõ ràng). ĐÃ CHUYỂN file về đúng
   đường dẫn admin/modules/settings/dashboard-settings.js để khớp với
   SCRIPT_SEQUENCE — nhớ XOÁ file cũ ở đường dẫn sai (nếu còn) để
   tránh nhầm lẫn/trùng lặp sau này.

   ⚠️ MỚI (đúng theo yêu cầu "khởi tạo mã sản phẩm / mã kho" phải là
   bước bắt buộc — nếu chưa khởi tạo thì không nhập kho được và không
   có công thức được): Mã sản phẩm (tab 📦 Sản phẩm) và Mã kho (tab
   🏬 Kho) giờ là trường BẮT BUỘC, có kiểm tra trùng mã phía client
   trước khi lưu. 2 file liên quan cũng được cập nhật ĐỒNG BỘ:
     - modules/inventory/dashboard-ingredients.js: modal Thêm/Sửa
       nguyên liệu ở trang "📦 Kho nguyên liệu" giờ CŨNG có trường Mã
       sản phẩm bắt buộc (trước đây modal ở đó KHÔNG có trường mã —
       chỉ modal ở Settings có, nên tạo nguyên liệu qua đường đó vẫn
       lách được yêu cầu "phải khởi tạo mã trước"). Nút "📥 Nhập/Điều
       chỉnh" bị khoá + báo rõ lý do nếu nguyên liệu chưa có mã.
     - modules/drinks/dashboard-drinks.js: dropdown chọn nguyên liệu
       trong công thức pha chế giờ CHỈ hiện nguyên liệu đã có mã sản
       phẩm (trừ đúng 1 ngoại lệ: nguyên liệu ĐANG được chọn sẵn ở
       dòng công thức cũ, để không làm mất dữ liệu công thức đã lưu
       trước đây — sẽ hiện kèm cảnh báo "CHƯA CÓ MÃ" ngay trong tên).

   ⚠️ Việc kiểm tra trùng mã ở đây chỉ là lớp bảo vệ PHÍA CLIENT (query
   trước khi insert/update) — vẫn có xác suất (hiếm) 2 người lưu trùng
   mã cùng lúc do không có transaction. Khuyến nghị bổ sung thêm ràng
   buộc UNIQUE cho cột `code` ở bảng `ingredients` và `branches` trên
   Supabase để có lớp bảo vệ chắc chắn ở tầng DB — không bắt buộc để
   tính năng này chạy được, nhưng nên làm.
  ...
  page.innerHTML = `
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">Khởi tạo mã kho &amp; mã sản phẩm — nền tảng bắt buộc cho Kho nguyên liệu, Đơn hàng &amp; Công thức pha chế</p>
      </div>
    </div>
    ...
    <div id="stTabWarehouses">
      <div style="font-size:12px;color:var(--text-muted);background:var(--bg);border-radius:10px;padding:10px 16px;margin-bottom:16px;">
        🔖 <b>Khởi tạo mã kho</b> để quản lý được nhiều địa điểm/kho khi phát sinh thêm — mỗi kho có 1 mã riêng, dùng để chọn khi tạo phiếu ở trang <b>📋 Kiểm kê tồn kho</b>.
      </div>
    ...
    <div id="stTabProducts" style="display:none;">
      <div style="font-size:12px;color:var(--text-muted);background:var(--bg);border-radius:10px;padding:10px 16px;margin-bottom:16px;">
        🔖 <b>Khởi tạo mã sản phẩm</b> là bước bắt buộc đầu tiên: sản phẩm phải có Mã thì mới (1) nhập/điều chỉnh được kho ở trang <b>📦 Kho nguyên liệu</b>, và (2) được chọn vào công thức pha chế ở trang <b>☕ Đồ uống</b>. Chưa có mã → 2 nơi đó sẽ không dùng được sản phẩm này.
      </div>
    ...
    <div id="stTabRecipes" style="display:none;">
      <div style="font-size:12px;color:var(--text-muted);margin-bottom:14px;">
        Tổng hợp công thức của mọi đồ uống. Bấm "✏️ Sửa công thức" để mở đúng công thức pha chế của món đó (dùng chung modal với trang ☕ Đồ uống). Món "⚠️ Chưa có công thức" là do trong công thức chưa chọn được nguyên liệu nào — kiểm tra lại nguyên liệu đó đã có <b>Mã sản phẩm</b> chưa ở tab 📦 Sản phẩm.
      </div>
      <div class="table-card">
        <table class="game-table">
...
function renderWarehousesTable() {
  ...
  tbody.innerHTML = list.map(b => `
    <tr style="${b.is_active === false ? 'opacity:.55;' : ''}">
      <td>${b.code ? `<b>${window.escHtml(b.code)}</b>` : '<span class="badge" style="background:#fff5f5;color:var(--danger);">⚠️ Chưa có mã</span>'}</td>
...
async function saveWarehouse() {
  ...
  /* ⚠️ MỚI: Mã kho giờ BẮT BUỘC — đúng vai trò "khởi tạo mã kho quản
     lý" là bước đầu tiên trước khi kho đó dùng được ở trang Kiểm kê
     tồn kho. */
  if (!code) { window.showFieldError("whCode", "Vui lòng nhập mã kho — đây là bước khởi tạo bắt buộc."); return; }
  window.clearFieldError("whCode");
  ...
  const payload = { code, name, is_active };

  const btn = document.getElementById("whSaveBtn");
  btn.disabled = true; btn.textContent = "Đang lưu...";
  try {
    /* ⚠️ MỚI: kiểm tra trùng mã kho phía client — branches không có
       deleted_at nên không cần lọc thêm điều kiện đó. */
    let dupQuery = client.from("branches").select("id").eq("code", code);
    if (id) dupQuery = dupQuery.neq("id", id);
    const { data: dupRow, error: dupErr } = await dupQuery.maybeSingle();
    if (dupErr) throw dupErr;
    if (dupRow) { window.showFieldError("whCode", "Mã kho này đã tồn tại — vui lòng dùng mã khác."); return; }
...
function renderProductsTable() {
  ...
  tbody.innerHTML = list.map(p => `
    <tr style="${p.is_active === false ? 'opacity:.55;' : ''}">
      <td>${p.code ? `<b>${window.escHtml(p.code)}</b>` : '<span class="badge" style="background:#fff5f5;color:var(--danger);">⚠️ Chưa có mã</span>'}</td>
...
function openAddProduct() {
  ...
  window.clearFieldError("prodCode");
...
async function saveProduct() {
  ...
  /* ⚠️ MỚI: Mã sản phẩm giờ BẮT BUỘC — đúng vai trò "khởi tạo mã sản
     phẩm" là bước đầu tiên, mở khoá nhập kho (Kho nguyên liệu) và
     công thức pha chế (Đồ uống) cho sản phẩm đó. */
  if (!code) { window.showFieldError("prodCode", "Vui lòng nhập mã sản phẩm — đây là bước khởi tạo bắt buộc trước khi nhập kho hoặc lên công thức.", { fullWidth: true }); return; }
  window.clearFieldError("prodCode");
  ...
  const staff = currentSession.displayName || currentSession.username;
  const payload = {
    code, name, spec: spec || null, unit, unit_cost,
  ...
  btn.disabled = true; btn.textContent = "Đang lưu...";
  try {
    /* ⚠️ MỚI: kiểm tra trùng mã sản phẩm phía client, chỉ tính trong
       số sản phẩm chưa bị xoá mềm. */
    let dupQuery = client.from("ingredients").select("id").eq("code", code).is("deleted_at", null);
    if (id) dupQuery = dupQuery.neq("id", id);
    const { data: dupRow, error: dupErr } = await dupQuery.maybeSingle();
    if (dupErr) throw dupErr;
    if (dupRow) { window.showFieldError("prodCode", "Mã sản phẩm này đã tồn tại — vui lòng dùng mã khác.", { fullWidth: true }); return; }
