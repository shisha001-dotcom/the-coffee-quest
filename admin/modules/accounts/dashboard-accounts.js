/* ══════════════════════════════════════════════
   ...
   - saveAccount(): các lỗi validate (thiếu username/password/username
...
/* ══════════════════════════════════════════════
   VALIDATE FIELD ERROR — MỚI (thay alert())
   ══════════════════════════════════════════════ */
...
async function saveAccount() {
  ...
  /* ⚠️ SỬA: lỗi validate báo ngay tại field thay vì alert() */
  ...
  try {
    ...
    /* ⚠️ SỬA: lỗi server báo qua toast thay vì alert(); lỗi đã xử lý
       tại field (username trùng) thì không cần toast thêm */
...
async function deleteAccount() {
  ...
  if (Number(id) === currentSession.id) {
    window.showToast('⚠️ Không thể tự xóa tài khoản đang đăng nhập.', '#e17055');
    return;
  }
